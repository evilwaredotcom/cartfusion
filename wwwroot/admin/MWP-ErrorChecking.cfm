...
<cfelse>
	<cfif NOT isDefined('Form.SearchCriteria')>
		...
		<cfabort>
	<cfelseif Form.SearchCriteria EQ 'ByEmail' AND (Form.EmailsProvided EQ '' OR Form.EmailsProvided CONTAINS ';')>
		<cfset ErrorMsg = 2>
		<cfinclude template="MWP-Home.cfm">
		<cfabort>
	<cfelseif Form.SearchCriteria EQ 'ByCriteria'>
		<cfif NOT isDefined('ByOrdAmtLeast') AND NOT isDefined('ByOrdAmtMost') AND NOT isDefined('ByOrdLeast') AND NOT isDefined('ByOrdMost')
			...
			AND NOT isDefined('ByState') AND NOT isDefined('ByCountry') AND NOT isDefined('ByIncludeEmails') AND NOT isDefined('ByEmail')>
			<cfset ErrorMsg = 3>
			<cfinclude template="MWP-Home.cfm">
			<cfabort>
