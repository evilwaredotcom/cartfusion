	...
	<cfif isDefined('form.ImportItemsFile') >
		...
		<cfloop query="resultsQuery">
			<cfif isDefined('resultsQuery.INVITEM')
			  ...
			  AND resultsQuery.INVITEMTYPE EQ 'INVENTORY'
			  AND TRIM(resultsQuery.NAME) NEQ 'PI'
				...
				<cfif FindProductMain.RecordCount EQ 0 >
					<cftry>
						<cfquery name="getMainCategoryID" dbtype="query">
							...
							<cfif TRIM(resultsQuery.PREFVEND) EQ 'Style' >
							WHERE	CatName = <cfqueryparam value="Style" cfsqltype="CF_SQL_VARCHAR">
							<cfelse>
							WHERE	CatName = <cfqueryparam value="#TRIM(ReplaceNoCase(resultsQuery.PREFVEND,'Style, ',''))#" cfsqltype="CF_SQL_VARCHAR">
							</cfif>
						</cfquery>
					...
					<cftry>	
						<cfif getMainCategoryID.RecordCount EQ 0 >
							<cfif TRIM(resultsQuery.PREFVEND) EQ 'Style' >
								<!--- <b>Style</b><br> --->
								<cfset MainCatToAdd = 'Style' >
							<cfelse>
								<!--- <b>#TRIM(ReplaceNoCase(resultsQuery.PREFVEND,'Style, ',''))#</b><br> --->
								<cfset MainCatToAdd = TRIM(ReplaceNoCase(resultsQuery.PREFVEND,'Style, ','')) >
							</cfif>
							...
							<cfquery name="insertMainCategory" datasource="#application.dsn#">
								...
								VALUES (
										...
										<cfqueryparam value="#TRIM(ReplaceNoCase(resultsQuery.NAME,'PI:',''))#.jpg" cfsqltype="CF_SQL_VARCHAR">
										)
					...
					<cftry>
						<cfquery name="getCategoryID" dbtype="query">
							SELECT	CatID
							FROM	getCategories
							<cfif TRIM(resultsQuery.PREFVEND) EQ 'Style' AND TRIM(resultsQuery.PURCHASEDESC) EQ '' >
							WHERE	CatName = <cfqueryparam value="Style" cfsqltype="CF_SQL_VARCHAR">
							<cfelse>
					...
					<cftry>	
						<cfif getCategoryID.RecordCount EQ 0 AND TRIM(resultsQuery.PURCHASEDESC) NEQ '' > <!---AND TRIM(resultsQuery.PREFVEND) NEQ 'Style'--->
					...
					<!--- <cftry> --->
						<cfquery name="InsertProductMain" datasource="#application.dsn#">
							...
							VALUES (
									<cfqueryparam value="1" cfsqltype="CF_SQL_INTEGER">,
									<cfqueryparam value="#TRIM(ReplaceNoCase(resultsQuery.NAME,'PI:',''))#" cfsqltype="CF_SQL_VARCHAR">,
						...
						<!--- <i>--- #ReplaceNoCase(resultsQuery.NAME,'PI:','')#</i><br> --->
				...
				<cfelse>
					<!--- <cftry> --->
						<cfquery name="UpdateProductMain" datasource="#application.dsn#">
							UPDATE	Products
							SET		SKU = <cfqueryparam value="#TRIM(ReplaceNoCase(resultsQuery.NAME,'PI:',''))#" cfsqltype="CF_SQL_VARCHAR">,
					...
					<cftry>
						<cfquery name="InsertCustomerMain" datasource="#application.dsn#">
							INSERT INTO Customers
								   (
									CustomerID, CustomerName, FirstName, LastName, CompanyName,
							...
							VALUES (
									...
									<cfelse>
										<cfqueryparam value="Ponsard#RandRange(1000,9999)#" cfsqltype="CF_SQL_VARCHAR">,
									</cfif>
