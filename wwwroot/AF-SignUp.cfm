<cfmodule template="tags/layout.cfm" PageTitle="Affiliate Sign Up" CurrentTab="Affiliates">
...
<cfif structKeyExists(form, 'RegisterAffiliate.x') AND structKeyExists(form, 'FirstName') AND form.FirstName NEQ ''>
	<cftry>
		...
		<cfif not checkDuplicate.RecordCount>
			...
			<cfscript>
				...
				if ( Form.Password NEQ '') Form.Password = ENCRYPT(Form.Password, application.siteConfig.data.CryptKey, "CFMX_COMPAT", "Hex") ;
				else Form.Password = '';
			...
			<!--- Added by Carl Vanderpal 28 May 2007 --->
			<cfscript>
...
<cfform action="AF-Signup.cfm" method="post" preservedata="yes" class="f-wrap-1">
	...
	<fieldset id="Address">
		...
		<label for="StateCode"><b><span class="req">*</span>State:</b>
			<cfselect query="getStates" name="State" value="StateCode" display="State" selected="#application.siteConfig.data.CompanyState#" size="1" required="yes" /><br />
		</label>
		...
		<label for="Country"><b><span class="req">*</span>Country:</b>
			<cfselect query="getCountries" name="Country" value="CountryCode" display="Country" selected="#application.siteConfig.data.CompanyCountry#" class="cfFormField" size="1" required="yes" /><br />
		</label>
	...
	<fieldset>
		...
		<label for="AffiliateText">
			...
			As an affiliate of <cfoutput>#application.siteConfig.data.DomainName#</cfoutput>, you will receive a percentage of the net sales
	...
	<div class="f-submit-wrap">
		<input type="submit" value="Submit" class="button" tabindex="12" /><br />
	</div>
	</fieldset>
	</cfform>
</div>
