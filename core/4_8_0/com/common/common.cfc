<cfcomponent displayname="Common Functions Module" hint="This component handles common function calls from a CartFusion site.">
	...
	<cffunction name="getAllProducts" displayname="Get All Products" hint="Function to retrieve all products information." access="public" returntype="query">
		...
		<CFQUERY NAME="getProducts" DATASOURCE="#variables.dsn#">
			...
			<!--- DISPLAY --->
			<cfif StructKeyExists(arguments,'CatDisplay') AND arguments.CatDisplay NEQ ''>
				AND 	(Category = #arguments.CatDisplay# 
				OR 		OtherCategories LIKE '%,#arguments.CatDisplay#,%')
			<cfelseif StructKeyExists(arguments,'SecDisplay') AND arguments.SecDisplay NEQ ''>
				AND 	(SectionID = #arguments.SecDisplay#
				OR 		OtherSections LIKE '%,#arguments.SecDisplay#,%')
			</cfif>
			
			<!--- FILTER --->
			<cfif StructKeyExists(arguments,'CatFilter') AND arguments.CatFilter NEQ ''>
				AND 	(Category = #arguments.CatFilter# 
				OR 		OtherCategories LIKE '%,#arguments.CatFilter#,%')
			<cfelseif StructKeyExists(arguments,'SecFilter') AND arguments.SecFilter NEQ ''>
				AND 	(SectionID = #arguments.SecFilter#
				OR 		OtherSections LIKE '%,#arguments.SecFilter#,%')
			</cfif>
				
			<!--- SHOW ME --->
			<cfif StructKeyExists(arguments,'SMC') AND arguments.SMC NEQ ''>
				AND ((
			...
			<cfelseif StructKeyExists(arguments,'SMS') AND arguments.SMS NEQ ''>
				AND ((
			...
			ORDER BY 
			<cfif structKeyExists(arguments,'SortOption')> DisplayOrder, #arguments.SortOption# <cfelse> Category, DisplayOrder, SKU </cfif>
			<cfif structKeyExists(arguments,'SortAscending') AND arguments.SortAscending EQ 1 > ASC <cfelse> DESC </cfif>
		</CFQUERY>
...
<!--- CART & WISHLIST FUNCTIONS --->
	...
	<cffunction name="login" hint="Logs customer into the system">
		...
		<cfquery name="login" datasource="#variables.dsn#">
			SELECT 	*
			FROM 	Customers
			WHERE 	Email = <cfqueryparam cfsqltype="cf_sql_longvarchar" value="#arguments.email#">
			AND 	UserName <> ''
			AND 	password <> ''
		</cfquery>
