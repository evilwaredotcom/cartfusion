...
<cfcomponent displayname="Books Module">
			...
			<cftransaction>
				...
				<cfif findThisListing.bSuccess AND arguments.Listings_BuyerPaymentType1 >
					<cfquery name="data" datasource="#variables.dsn#">
						...
						<!--- deduct from bucks account --->
						<cfelseif arguments.Listings_BuyerPaymentType1 EQ 2 >
						UPDATE		Customers
						SET			Customers_Bucks = (Customers_Bucks - #arguments.Listings_BuyerPaymentAmount1#)
						WHERE		Customers_CustomerID = '#arguments.Listings_BuyerCustomerID#'
						</cfif>
					</cfquery>
					...
					<cfscript>
						...
						// update customer's session variables with new bucks amount
						else if ( arguments.Listings_BuyerPaymentType1 EQ 2 )
							session[application.SessionVar].Customers_Bucks = session[application.SessionVar].Customers_Bucks - arguments.Listings_BuyerPaymentAmount1 ;
	...
	<cffunction name="ConvertListing" output="false">
		...
		<cftry>
			...
			<cfscript>
				stReturn.bSuccess = True ;
				stReturn.Message = '--- Your listing has been sold successfully ---' ;
			</cfscript>
				
			<cfcatch>
				<cfscript>
					stReturn.bSuccess = False ;
					stReturn.Message = '--- ERROR: We were unable to sell your listing. Please contact our support team. ---' ;
					stReturn.stError = cfcatch ;
				</cfscript>
			</cfcatch>
